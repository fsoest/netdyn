...
def collect_x_data(env, x_data, q_data, T, cars, CS):
    ...
    for i in range(T):
        ...
        yield env.timeout(1)
# %%
def parameters(N, charging_speed, max_charge, patience, range_anxiety, plot=False, plot2=False, plot3=False, L=100, Super=[]):
    env = PosEnvironment()
    CS = []
    for i in range(L):
        CS.append(charging_station(i, env, 1))
    for charger in Super:
        CS[charger] = charging_station(charger, env, 1000)
    cars = []
    name = range(N)
    ...
    if plot==True:
        ...
        plt.hlines(Super, 3000, 4000, colors='orange')
...
T = 5000
np.random.seed(20)
parameters(250, 1, 10, 1, 3, plot=True, Super=[50])

# %%
np.random.seed(20)
parameters(250, 1, 10, 1, 3, plot=True)
